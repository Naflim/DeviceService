...
namespace DeviceService.DeviceModel
{
    public class UHFReaderRFID : IReadOrWrite
    {
        ...
        {
            var tryBreak = mode switch
            {
                ConnectMode.Tcp => throw new UHFRFIDException("暂不支持网口连接"),
                ConnectMode.SerialPort => UHFRFIDSDK.UHF_RFID_Close((byte)com),
                _ => throw new UHFRFIDException("意外的连接模式"),
            };
            if (tryBreak != 0)
